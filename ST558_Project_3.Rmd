...
```{r}
modelLm <- lm(logShares~. ,data = trainDf)
...
## Linear Model Visulaization (Residual Plot)
...
ggplot(data = testDf, aes(x = lmPredict, y = log(shares))) +
  ...
  geom_line(aes(x = log(shares),
            ...
            linetype = 2, size = 1)+
  labs(x = 'Predicted Shares',
       y = 'Actul log(Shares)',
       title = paste("For", title))
...
```    
In ideal scenario, residuals must be centered around the zero line to fulfill the linear regression model assumptions. From the plot we can fairly assume that the residuals are centered around the zero mean.  
...
modelComparisonDf = tibble(model = c("Linear Model"), model_type = c(model_types[1]), RMSE = c(round(lmRes[[1]],2)), R_Squared = c(round(lmRes[[2]], 4)))
