...
modelLm <- train(log(shares) ~ .,
              data = trainDf %>% 
              dplyr::select(all_of(cols), shares),
              method = "lm",
              preProcess = c('center'),
...
```    
In ideal scenario, residuals must be centered around the zero line to fulfill the linear regression model assumptions. From the plot we can fairly assume that the residuals are centerd aroud the zero mean.  
...
fitLASSO <- train(log(shares) ~ . ,data=trainDf,
                  method    = "glmnet", 
...
predLASSO <- predict(fitLASSO, newx = log(testDf$shares))
rmse <- (mean((testDf$shares - predLASSO)^2))^0.5
rmse



#
# fitLASSO <- train(log(shares) ~. , data = trainDf %>% select(cols, shares), 
#                   method = "lm",
#                   preProcess = c("center", "scale"), 
