...
PCs <- prcomp(newspopData %>% select(-shares, -day, -title_Sentiment_Class), center=TRUE, scale=TRUE)
#pca
...
TreeModelData <- predict(PCs, newdata = newspopData) %>% 
  as_tibble() %>% 
  select(PC1:paste0("PC", pt))

TreeModelData$shares <- newspopData$shares
TreeModelData
```
...
```{r}
set.seed(200)
trainIndex <- createDataPartition(TreeModelData$shares, p = 0.7, list = FALSE)

trainData <- TreeModelData[trainIndex, ]
testData <- TreeModelData[-trainIndex, ]
...
randomForest <- train(shares ~., data = trainData, 
                      method = "rf",
                      trControl=trctrl,
                      preProcess = c("center", "scale"),
...
```{r}
rfPredict <- predict(randomForest, newdata = testData)
